...
class myfoxv2 extends eqLogic {
  ...
  /* public function cronHourly($_eqlogic_id = null)
    {
      $frequence = config::byKey('myfoxv2', 'pull');
        if ($frequence == '60min') {
           //self::executeinfo();
          'myfoxv2'::deamon_start();
         //  self::executeinfo('status daemon du plugin : ' . 'myfoxv2'->deamon_info()['state']);
        }
    }*/
   ...
   if (is_object($eqLogic) && $eqLogic->getIsEnable() == 1) {		
        ...
        is_object('myfoxv2')->deamon_start();
        }
      }
    }
   ...
   public function postSave() {
	...
	public function postUpdate() {
       ...
       /* if ($this->getConfiguration('cameraCount') > '0') {
		$myfoxData=myfoxv2::getDeviceType('camera');
		
		foreach ($myfoxData as $key => $v1) {
		
		$camera = $this->getCmd(null, 'camera'.$v1["deviceId"]);
		if (!is_object($camera)) {
			$camera = new myfoxv2Cmd();
